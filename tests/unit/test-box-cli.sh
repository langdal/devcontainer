...
assert_contains "$dock" "docker info" "docker mode waits for dockerd to be ready"

# the old entrypoint-hijack mechanism must be gone (it wedged msb exec)
assert_eq "" "$(echo "$dock" | grep -o 'entrypoint boxdockerd' || true)" "docker mode does not override the entrypoint"
...
assert_contains "$out_marker" "--init auto" ".box-docker enables docker mode"
