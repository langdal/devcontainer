...
if ! ./dev exec --dind -- docker version >/dev/null 2>&1; then
    log_fail "dev exec --dind -- docker version did not succeed"
    exit 1
fi
end=$(date +%s)
elapsed=$((end - start))
...
#
# These checks run as one-shot `./dev exec --dind -- git config ...`
...
# `./dev exec --dind -- ...` always lands as vscode instead — either by
...
restore_git_identity() {
    ...
    ./dev exec --dind -- git config --global --unset-all user.name  </dev/null >/dev/null 2>&1 || true
    ./dev exec --dind -- git config --global --unset-all user.email </dev/null >/dev/null 2>&1 || true
    rm -f "$_s22_cfg"
}
...
in_name=$(./dev exec --dind -- git config --global user.name </dev/null 2>/dev/null | tr -d '\r')
...
if ! ./dev exec --dind -- git config --global user.name "in-container-override" </dev/null >/dev/null 2>&1; then
    log_fail "failed to plant in-container git identity override"
    exit 1
fi
"$RUNTIME" rm -f "$D" >/dev/null 2>&1
if ! ./dev exec --dind -- true >/dev/null 2>&1; then
    log_fail "dev exec --dind restart after planting in-container identity failed"
    exit 1
fi
in_name_after=$(./dev exec --dind -- git config --global user.name </dev/null 2>/dev/null | tr -d '\r')
